...
# ================================================================ PART 2
md("""## Part 2 — Download Real Images and Identity File (fully automatic)

### 2.1 Download CelebA images (same images as the FaceMorph_EClub_Task set — 202,599 real faces)
""")
...
print("Download path:", DATA_ROOT)
print("Available real images:", len(all_jpgs))
print("Images folder:", IMG_DIR)
""")

md("""### 2.2 Identity file — no manual steps needed

To guarantee that the same person never appears in both training and testing,
we need `identity_CelebA.txt` (maps each image to a person ID). Three methods
are tried automatically, in order:
1. Search inside the files downloaded from Kaggle.
2. Auto-download the official `identity_CelebA.txt` from the public CelebA Google Drive via `gdown`.
3. **Guaranteed fallback**: treat every image as its own identity (pseudo-identities) —
   everything still runs; the identity-disjoint guarantee becomes image-level.
""")
code(r"""identity_path = None

# Method 1: search inside the downloaded files
for cand in glob.glob(os.path.join(DATA_ROOT, "**", "*identity*"), recursive=True):
    if os.path.isfile(cand):
        identity_path = cand
        print("✅ Identity file found inside the Kaggle download:", cand)
        break

# Method 2: auto-download from the official public CelebA link
if identity_path is None:
    try:
        ...
        if os.path.exists(out) and os.path.getsize(out) > 1_000_000:
            identity_path = out
            print("✅ identity_CelebA.txt auto-downloaded from the official source")
    except Exception as e:
        print("Auto-download failed:", e)

# Parse the file, or activate the fallback
if identity_path:
    ...
    print(f"Images: {len(ident)} — distinct identities: {ident['identity'].nunique()}")
else:
    # Method 3: pseudo-identities (each image = its own person) — always works
    names = sorted(os.path.basename(p) for p in all_jpgs)
    ident = pd.DataFrame({"image_id": names,
                          "identity": np.arange(len(names))})
    print("⚠️ Identity file unavailable — pseudo-identities activated automatically")
    print("   (split separation becomes image-level; all steps still work)")
...
src1 = cv2.imread(os.path.join(IMG_DIR, a))
src2 = cv2.imread(os.path.join(IMG_DIR, b))
